                ...
                def _update_epoch(update_state, epoch_index):
                    def _update_minbatch(update_carry, scan_input):
                        ...
                        def _compute_loss_components(
                            ...
                            reliability_std = jnp.sqrt(
                                masked_mean(
                                    jnp.square(
                                        reliability_scores - reliability_mean
                                    ),
                                    score_active_mask,
                                )
                            )
                            has_active_scores = jnp.any(score_active_mask)
                            reliability_min = jnp.where(
                                has_active_scores,
                                jnp.min(
                                    jnp.where(
                                        score_active_mask,
                            ...
                            def _masked_axis_mean(values, mask, axes):
                                count = jnp.sum(mask, axis=axes)
                                return jnp.sum(
                                    jnp.where(mask, values, 0.0),
                                    axis=axes,
                                ) / jnp.maximum(count, 1.0)

                            reliability_level_mean = _masked_axis_mean(
                                reliability_scores,
                                score_active_mask,
                                (0, 1, 3, 4),
                            )
                            ...
                            reliability_side_mean = _masked_axis_mean(
                                reliability_scores,
                                score_active_mask,
                                (0, 1, 2, 4),
                            )
